...
async function renderNotifications() {
  ...
  const ns = await api(`/api/notifications?email=${store.user.email || ''}`);
  if (!ns || ns.length === 0) {
    list.innerHTML = "<p style='color:var(--muted)'>No new notifications</p>";
    return;
  }
...
function navigate(view) {
  ...
  if (store.counselor.authenticated) {
    const allowed = new Set(['counselor-dashboard','settings','counselors']);
    if (!allowed.has(view)) view = 'counselor-dashboard';
  }
...
function ensureAuthUI() {
  ...
  if (isCounselor && !isUserOrAdmin) {
    document.querySelectorAll('nav .nav-btn').forEach(b => {
      const v = b.dataset.view;
      const show = v === 'settings' || v === 'counselors';
      b.style.display = show ? '' : 'none';
    });
  } else {
    document.querySelectorAll('nav .nav-btn').forEach(b => { b.style.display = '' });
  }
  const isAnyAuth = isUserOrAdmin || isCounselor;
  if (!isAnyAuth) navigate('landing-login');
}
...
async function renderCounselorAppointments() {
  const root = document.getElementById("c-appointments");
  if (!root) return;
  root.innerHTML = "";
  ...
  if (!aps || aps.length === 0) { const li = document.createElement("li"); li.textContent = "Demo • client@example.com • 2026-01-05 10:00 • pending"; root.appendChild(li); return }
  aps.forEach(a => {
    const li = document.createElement("li");
    li.innerHTML = `${fmt(a.ts).date} • ${a.userEmail || ''} • ${a.date} ${a.time} • ${a.status}
      <div style="display:flex;gap:8px;margin-top:6px;">
        <button class="btn-formal" onclick="acceptAppointment(${a.id})">Accept</button>
        <input type="text" id="zoom-${a.id}" placeholder="Zoom link" value="${a.zoomLink || ''}" style="flex:1;">
        <button class="btn-formal" onclick="sendZoomLink(${a.id})">Send Link</button>
      </div>`;
    root.appendChild(li);
  });
}
...
async function renderCounselorPayments() {
  const root = document.getElementById("c-payments");
  if (!root) return;
  root.innerHTML = "";
  const cid = store.counselor.id;
  if (!cid) { root.textContent = "No counselor logged in"; return }
  const pays = await api(`/api/payments?cid=${cid}`);
  if (!pays || pays.length === 0) { const li = document.createElement("li"); li.textContent = "Demo • Week package • John Doe • ****4242"; root.appendChild(li); return }
  pays.forEach(p => {
    const li = document.createElement("li");
...
async function renderCounselorFeedback() {
  const root = document.getElementById("c-feedback-list");
  if (!root) return;
  root.innerHTML = "";
  const cid = store.counselor.id;
  if (!cid) { root.textContent = "No counselor logged in"; return }
  const items = await api(`/api/feedback?cid=${cid}`);
  if (!items || items.length === 0) { const li = document.createElement("li"); li.textContent = "Demo • \"Great session, very helpful\" • Sentiment: POSITIVE"; root.appendChild(li); return }
  items.forEach(f => {
    const li = document.createElement("li");
    li.textContent = `${fmt(f.ts).date} • ${f.sentiment || 'neutral'} • ${f.comment}`;
    root.appendChild(li);
  });
}
...
async function book(e) {
  ...
  await api('/api/notifications', 'POST', {
    msg: `Your appointment with ${counselorName} on ${date} at ${time} is Pending.`,
    status: 'pending'
  });
  ...
  setTimeout(async () => {
    ...
    const msg = accepted
      ? `Your appointment with ${counselorName} on ${date} at ${time} has been ACCEPTED.`
      : `Your appointment with ${counselorName} on ${date} at ${time} was DECLINED.`;
    ...
    // EMAIL: User Acceptance/Decline
    await api('/api/emails', 'POST', {
      sender: counselorName,
      recipient: store.user.email,
      subject: `Appointment ${accepted ? "Accepted" : "Declined"}`,
      body: `Dear User,\n\n${msg}\n\nBest regards,\n${counselorName}`
    });

    if (document.getElementById("notifications").classList.contains("active")) renderNotifications();
    alert(`New Notification & Email: ${accepted ? 'ACCEPTED' : 'DECLINED'}`);
  }, 3000);
}
...
function setupPasswordToggles() {
  ...
  passwordInputs.forEach(input => {
    ...
    btn.style.cssText = "position:absolute; right:10px; background:none; border:none; padding:4px; display:flex; align-items:center; justify-content:center; color:#32de84; cursor:pointer; z-index:20;";
    btn.innerHTML = `<svg class="eye-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#32de84" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg><svg class="eye-off-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#32de84" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:none;"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>`;
